<template>
  <div>
    ...
    <ul class="flex-container">
      <li v-for="item in items.results" :key="item.id" class="flex-item" @click.prevent="exploremore(item)">
        <div id="image">
          <img v-if="getImg(item)" :src="getImg(item).url" />
        </div>
...
export default {
  ...
  methods: {
    ...
    exploremore(item) {
      ...
      router.push({
        name: 'article',
        params: { article: JSON.stringify(item.details_en) },
      });
      // let routeData = this.$router.resolve({ name: 'article', params: { article: JSON.stringify(item.details_en) }})
